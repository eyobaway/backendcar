...
const seed = async () => {
    try {
        ...
        await database_1.default.sync(); // Sync without force: true
        console.log('Database connected and schema synced!');
        const hashedPassword = await bcryptjs_1.default.hash('password123', 10);
        // helper to get or create user
        const getOrCreateUser = async (data) => {
            let user = await models_1.User.findOne({ where: { email: data.email } });
            if (!user) {
                user = await models_1.User.create(data);
            }
            return user;
        };
        // Create Admin User
        const adminPassword = await bcryptjs_1.default.hash('admin', 10);
        ...
        const user1 = await getOrCreateUser({ name: 'Abebe Kebede', email: 'abebe@realtor.com', password: hashedPassword, role: User_1.UserRole.AGENT, profileImage: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?q=80&w=1887&auto=format&fit=crop' });
        const user2 = await getOrCreateUser({ name: 'Solomon Tekle', email: 'solomon@realtor.com', password: hashedPassword, role: User_1.UserRole.AGENT, profileImage: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1887&auto=format&fit=crop' });
        const user3 = await getOrCreateUser({ name: 'Hirut Tadesse', email: 'hirut@realtor.com', password: hashedPassword, role: User_1.UserRole.AGENT, profileImage: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=1887&auto=format&fit=crop' });
        const user4 = await getOrCreateUser({ name: 'Dawit Getachew', email: 'dawit@realtor.com', password: hashedPassword, role: User_1.UserRole.AGENT, profileImage: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=1888&auto=format&fit=crop' });
        const getOrCreateAgent = async (data) => {
            let agent = await models_1.Agent.findOne({ where: { userId: data.userId } });
            if (!agent) {
                agent = await models_1.Agent.create({ ...data, isActive: true });
            }
            return agent;
        };
        ...
        const propertyData = [
            {
                ...
                condition: 'NEW',
                bodyType: 'Sedan',
                type: Property_1.PropertyType.SALE,
            ...
            },
            {
                ...
                condition: 'NEW',
                bodyType: 'Coupe',
                type: Property_1.PropertyType.RENT,
                rentCycle: 'DAILY',
        ...
        for (const prop of propertyData) {
            const existing = await models_1.Property.findOne({ where: { title: prop.title } });
            if (!existing) {
                await models_1.Property.create(prop);
            }
        }
        // Add Articles
        const existingArticle = await models_1.Article.findOne({ where: { title: 'Top 5 Tips for Buying a Used Car in 2024' } });
        if (!existingArticle) {
            await models_1.Article.create({
